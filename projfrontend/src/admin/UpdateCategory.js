...
export default function UpdateCategory(props) {
  ...
  const onSubmit = (e) => {
    ...
    updateCategory(props.match.params.categoryId,user._id, token, { name: values.name }).then((data) => {
      if (data.error) {
  ...
  const successMessage = () => {
    return (
      <div className="row">
      <div className="col-md-6 offset-sm-3 text-left">
        <div className="alert alert-success" style={{display : values.success ? "" : "none"}}>Category updted Successfully.</div>
      </div>
      </div>
